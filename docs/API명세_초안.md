...
> **작성일** 2026.07.30 · **현행성 점검** 2026.07.31 · 설계 근거는
> [`review/API설계_사전결정.md`](review/API설계_사전결정.md)
>
> **이 문서는 미구현 API의 착수용 초안입니다.** FastAPI가 생성하는 OpenAPI(`/docs`)가
> 구현 정본이며 산출물 제출 대상이 아닙니다.
>
> 도출 근거 — `db/schema.sql`(정본) · 02 유스케이스 14건 · 화면 15개([`review/화면설계서_개정안.md`](review/화면설계서_개정안.md))
>
> **현재 구현** — 전체 계약 34개 중 외부 인증 API 6개
> (`signup`, `check-email`, `login`, `logout`, 비밀번호 재설정 요청·확정)
