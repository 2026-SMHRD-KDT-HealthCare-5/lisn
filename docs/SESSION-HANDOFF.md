...
2. **프론트 연동** — 백엔드가 다 열렸으니 앱·관리자 웹의 목업을 실제 API 로 교체.
   ...
   > **2026.08.01 결정 2건이 앱 작업에 걸립니다.**
   > - **PDF 는 앱이 만듭니다.** `GET /reports/export` 는 없습니다. `GET /reports` 응답으로
   >   조판하세요. 기간·생성일시·본인 식별 정보는 **기기와 무관하게 같은 위치**에
   > - **음성 입력은 범위 밖입니다.** 마이크 버튼이 있으면 지우거나 「확장 예정」 표기.
   >   눌리는데 동작하지 않으면 시연에서 드러납니다
