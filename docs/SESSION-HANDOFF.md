...
> **문서·화면 작업은 남은 게 없습니다.** 아래 넷은 전부 **실기기나 조직
> 설정이 필요한 것**이라, 더 열심히 한다고 빨라지지 않습니다. 실기기부터
> 확보하세요.

| # | 항목 | 담당 | 상태 |
|---|---|---|---|
| 1 | **Health Connect 실기기 검증** ⭐⭐ | 윤일준·함은선 | 구현 완료, **실기기 필요** |
| 2 | **`SD-B①` 03 학습 방법 교체** ⭐ | 김건영·이응균 | 문안 준비됨 |
| 3 | `MUSIC`·`FOOD` 콘텐츠 | 팀 | 진행 중 |
| 4 | 실기기 QA | 전원 | 1 이후 |
| 5 | `LLM_PROVIDER=openai` 전환 검증 | 이응균 | **막힘** |
...
#### 1. 화면설계서 슬라이드 6장 ⭐⭐ — 가장 급합니다

**신규 6개 화면이 전부 구현됐는데 설계서에만 없습니다.** 심사에서 「구현 근거가 되는
설계가 없다」로 걸리는 조합입니다.

명세 문안과 시안 PNG 는 **다 준비돼 있습니다.** 슬라이드를 넣는 작업만 남았습니다.
...
[`문서개정_체크리스트.md`](review/문서개정_체크리스트.md) 에 문안 그대로 있습니다.
...
지금까지 검증은 **전부 Android 에뮬레이터** 기준입니다. 1·2 가 끝난 뒤 진행합니다.

#### 6. `LLM_PROVIDER=openai` 전환 — 막혀 있습니다
