...
**앱 화면 14개와 관리자 웹 2개가 모두 실제 API 에 붙었습니다.** 목업 데이터는 남아 있지
않습니다. 화면설계서 PPTX 도 21장으로 개정이 끝났습니다.
남은 것은 **Health Connect 실기기 검증**입니다.

> ⚠ 2026.08.02 에 **에뮬레이터로는 보이지 않던 결함 셋**을 고쳤습니다 — 평문 HTTP 가
> 릴리스에서만 차단되고, `tel`·`https` 는 Android 11+ 가시성 선언이 필요합니다.
> 실기기로 붙일 때는 [`frontend/app/README.md`](../../frontend/app/README.md)
> 「실기기」 절을 먼저 읽으세요.
...
| 파일 | 왜 |
|---|---|
| `docs/결정/화면설계서_개정안.md` | **화면 15개의 명세.** Part D 에 현재 앱 파일과의 대응표 |
| `docs/결정/API명세_초안.md` | 엔드포인트 34개. 요청·응답 스키마 |
| `docs/결정/API설계_사전결정.md` | 왜 그렇게 설계했는지. 바꾸기 전에 반드시 확인 |
