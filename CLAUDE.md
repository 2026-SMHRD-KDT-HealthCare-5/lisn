...
> **현행성 점검** 2026.07.31 · 현재 개발 브랜치는 `함은선`이며 관리자 인증·긴급 상담과
> 루트 통합 실행 환경까지 구현된 상태입니다.
...
Flutter의 인증 API 6개·보안 토큰 저장·인증 게이트와 `MAIN_EMERGENCY_01` 긴급 상담 화면이
구현됐습니다. 관리자 웹은 같은 로그인 API를 사용하며 `ADMIN` 역할만 세션을 허용합니다.
홈·챗봇·라이프로그·설정의 업무 데이터와 관리자 대시보드 데이터는 백엔드 라우터가 완성되는
순서대로 연동해야 합니다.
