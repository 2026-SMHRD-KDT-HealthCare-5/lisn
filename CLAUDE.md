...
> **현행성 점검** 2026.08.04 · OpenAI 가 풀려 평가셋을 전량 채점했고
> `NFR-DV-001`·`NFR-TS-001` 도 **둘 다 통과**했습니다. 한편 **기획서가
> 분석정의서와 반대 결론을 들고 있는 것**이 드러나 개정 28건이 새로
> 생겼습니다(12건 반영, 8/11 마감).
>
> **평가셋이 닫혔습니다**(8/5) — 211건 · 위기 111 · 비위기 100 · `team` 211/211 ·
> 교차 라벨링·조정 완료. **재현율 0.793 · 정밀도 0.967 · F1 0.871** 입니다.
> ⚠ **`0.919` 를 쓰지 마세요** — AI 초안으로 잰 값이라 「사람이 작성」
> 요건을 못 채웁니다. 드러나면 방어할 수 없습니다.
>
> **지금 가장 급한 것** — **HWP 개정이 8/5 에 닫혔습니다** — 요구사항정의서 12건 · 기획서 · 빅데이터분석정의서 4건 반영 완료. **문서·평가셋 전부 닫혔습니다**(8/5). 남은 문서는 `PL-38` 한 건이고, **이제 구현입니다** — 앱에 선제 접촉 연결 · Health Connect 실기기 · FCM. 그 밖에 Health Connect 실기기 검증과 FCM 발송
> 경로(앱 토큰 발급 · 서버 발송)가 남았습니다.
>
> ⚠ **발표자료 PPTX 는 개정 범위에서 뺐습니다**(2026.08.04 PM 결정). 기획서
> 발표는 끝났고, `PL-20`~`PL-24` 는 **8/28 최종 발표 준비 시점으로 이월**합니다.
> 그래서 `tools/check_docs.py` 는 **0 이 되지 않습니다** — 발표자료 몫 4건이
> 계속 잡히니 **요구사항정의서와 기획서만 보세요.**
