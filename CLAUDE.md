...
> **현행성 점검** 2026.08.03 · OpenAI 가 활성화되어 막혀 있던 것이 풀렸습니다.
> 평가셋 200건을 전량 채점했고(F1 0.614 → **0.919**), `NFR-DV-001`·`NFR-TS-001`
> 도 측정해 **둘 다 통과**했습니다. 한편 **01 기획서가 03 분석정의서와 반대
> 결론을 들고 있는 것**이 드러나 개정 26건이 새로 생겼습니다(8/11 마감).
>
> **지금 가장 급한 것 셋** — ① 평가셋 **180건이 AI 초안**이라 사람 검수 전에는
> 성적으로 인용할 수 없습니다 ② 발표자료 KPI 타일에 달성 못 한 `85%`·`0.80`
> 이 떠 있습니다 ③ 기획서 개정. 그 밖에 Health Connect 실기기 검증과 FCM
> 발송 경로(요구사항 3건이 묶임)가 남았습니다.
...
- **`[확인]` 이라고 다 결정 대기가 아닙니다.** 문서가 이미 요구하는 것을 확인 항목으로
  잘못 분류한 적이 있습니다. `height_cm`은 04·05에서 확정된 뒤 `MAIN_JOIN_02` 선택
  입력으로 구현까지 완료됐습니다.
...
- **`USERS.role`** (`SD-E1`) — 관리자 기능이 요구사항 5곳에 있는데 데이터 모델에 관리자 개념이 없었습니다. `role VARCHAR(20) DEFAULT 'USER' CHECK IN ('USER','ADMIN')` 추가. `schema.sql`·04·05 HWP 모두 반영 완료됐습니다.
