...
| 코드 | 되돌리면 안 되는 이유 |
|---|---|
| **OpenAI 경로 유지** | Gemini 는 임시 수단입니다. 산출 문서(01 · 02 `MLCM_310`/`MLCM_320`)의 "외부 OpenAI API" 가 정본이고 **문서는 고치지 않습니다.** 지우면 문서와 실제가 어긋납니다 |
...
`risk_level_of()`(`ai/server/main.py`)는 모델이 아니라 **정책**(04 문서 6항)입니다.
