...
> ## ⚠ 이미 한 번 배포했습니다 (2026.08.24) — 처음부터 다시 하지 마세요
>
> `lisn-server`(공인 IP `101.79.24.15`)가 이미 있고, VPC·Subnet·ACG·인증키
> 전부 만들어져 있습니다. **1~4절(NCP 콘솔 생성)은 다시 할 필요 없습니다.**
> 지금은 **요금 절약을 위해 정지 상태**입니다.
>
> 이어가려면 [`docs/SESSION-HANDOFF.md` §3-2](../docs/SESSION-HANDOFF.md#32-클라우드-배포-20260824)
> 부터 읽으세요 — 지금 뭐가 됐고 다음이 뭔지, SSH 키가 이 PC 에만 있다는
> 문제와 그 우회 방법이 거기 있습니다. 정말 서버를 새로 만들어야 하는
> 상황(예: 이 서버를 실수로 반납함)이면 그때만 1절부터 다시 따라가세요.
