...
> `model_version` 이 `rule-` 로 시작하면 **모델 결과가 아닙니다**(현재
> `rule-baseline-v1`). 이 값으로 성능을 주장하지 마세요.
...
| 코드 | 상황 |
|---|---|
| 404 | 최근 14일 라이프로그 **행이 없음** |
| 422 | 행은 있으나 **지표가 비어** 기준값을 만들 수 없음 (실측 3일치 미만) |
| 503 | DB 연결 실패 |

> ### ⚠ 데이터가 없을 때 `NORMAL` 을 쓰지 않습니다
>
> 권한만 승인하고 실제 수집이 안 된 상태에서 판정을 강행하면 편차가 0 이라 `NORMAL`
> 이 나옵니다. 그건 「정상」이 아니라 **「모름」**입니다. 거짓 정상 기록이 남으면
> 사용자 대시보드와 관리자 관제(`MLCM_501`) 양쪽에서 위험을 놓칩니다. 적재하지 않고
> 422 로 끊습니다.
>
> `_predict()` 를 모델로 교체할 때 이 가드(`_has_signal()`)를 같이 들어내지 마세요.
...
외부에 열지 않는 것이 전제입니다**(`docs/결정/API설계_사전결정.md` 7절).
...
위험 단계 매핑은 **모델이 아니라 정책**입니다(데이터베이스요구사항분석서 6항). `_predict()` 를 교체할 때
이 함수까지 같이 들어내지 마세요.
...
`docs/결정/API명세_초안.md` 의 내부 API 절도 이 구조로 고쳐 **철회 표기**했습니다(2026.08.01).
...
DB 없이 도는 **정책 테스트**입니다. `risk_level_of()` 의 매핑과 `_has_signal()` 의
판정 거부 조건을 고정합니다. `_predict()` 를 모델로 교체해도 이 테스트는 그대로
통과해야 합니다 — **통과하지 않으면 정책을 같이 들어낸 것입니다.**
