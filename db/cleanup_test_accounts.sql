...
DELETE FROM USERS u
 WHERE
   ...
   -- ⚠ **관제 코호트(seed_demo_cohort.sql)도 남깁니다.**
   --   그 시드의 `cohort.09` 는 **일부러 판정을 안 넣은 미평가 대상자**라
   --   아래 「데이터가 붙어 있으면 남긴다」 조건에 안 걸리고 지워집니다.
   --   지우면 관제 화면의 「미평가 N명은 위험이 없다는 뜻이 아닙니다」
   --   안내가 사라져, 시연에서 그 배려를 설명할 수 없게 됩니다.
   --   (2026.08.22 시연 준비 중 실제로 지워질 뻔했습니다)
   AND u.email NOT LIKE 'cohort.%'
