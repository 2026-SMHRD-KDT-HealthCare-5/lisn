...
--             ⚠ 다시 넣으면 user_id 가 바뀌어 **앱에 로그인해 둔 세션이 끊깁니다.**
--               DEV_LOGIN=force 로 다시 띄우면 됩니다.
--
--  ⚠ **날짜가 바뀌면 다시 실행하세요.**
--    collected_at 을 now() 기준 상대값으로 넣기 때문에, 자정을 넘기면 이 시드의
--    가장 최근 기록이 「어제」가 됩니다. 홈 화면은 **오늘** 라이프로그를 보여주므로
--    수면·걸음·HRV 가 전부 「-」로 나옵니다. 화면이 깨진 게 아니라 데이터가 없는
--    것인데, 원인을 찾는 데 시간이 걸립니다(실제로 한 번 헤맸습니다).
--
--  ---------------------------------------------------------------------
--  ⚠ 이건 만들어낸 데이터입니다. 절대 성능 근거로 쓰지 마세요.
--
--    model_version 을 'seed-demo-v0' 로 박아 뒀습니다. 실제 판정 결과
--    ('rule-baseline-v1' 또는 이후의 모델 버전)와 **구분되도록** 하기
...
INSERT INTO LIFELOG_METRICS (
    ...
    awake_min, sleep_efficiency_pct, heart_rate, hrv,
    sleep_start_at, sleep_onset_min, activity_start_at, collected_at
)
SELECT
    u.user_id,
    ...
    -- 입면 시각: 전날 밤. sleep_min 만큼 자고 그날 아침에 깬 것으로 둡니다.
    date_trunc('day', now()) - make_interval(days => d.days_ago + 1)
        + make_interval(mins => d.sleep_at_min),
    d.onset_min,
    -- 활동 개시 시각: 그날 아침
    date_trunc('day', now()) - make_interval(days => d.days_ago)
        + make_interval(mins => d.act_at_min),
    date_trunc('day', now()) - make_interval(days => d.days_ago) + INTERVAL '9 hours'
FROM USERS u
CROSS JOIN (VALUES
    -- ⚠ 시각 3컬럼은 2026.08.03 에 추가했습니다. 개인 기준선 이탈 탐지가
    --   총량(수면·걸음)뿐 아니라 **타이밍**을 보기 때문입니다. 없으면
    --   MLCM_220 선제 접촉이 잡아야 할 신호가 데모에 안 나타납니다.
    --
    --   sleep_at_min  전날 자정 기준 분 (1380 = 23:00)
    --   act_at_min    그날 자정 기준 분 (450 = 07:30)
    --
    --      d전,걸음,거리,칼로리,활동,수면,깊은,얕은,렘,깸,효율,심박,HRV,입면시각,입면지연,활동개시
    (13,  7420, 5100, 2180, 68, 421, 92, 236,  78, 15, 93.5, 66, 48.2, 1380, 12, 450),
    (12,  6980, 4820, 2050, 61, 408, 88, 229,  75, 16, 92.8, 67, 47.1, 1395, 14, 455),
    (11,  7810, 5390, 2240, 74, 433, 95, 244,  80, 14, 94.1, 65, 49.6, 1370, 11, 445),
    (10,  7140, 4930, 2110, 65, 415, 90, 232,  77, 16, 93.0, 66, 47.8, 1385, 13, 450),
    ( 9,  6650, 4590, 1980, 58, 396, 84, 222,  73, 17, 91.9, 68, 45.3, 1400, 15, 460),
    ( 8,  7020, 4840, 2070, 63, 402, 86, 226,  74, 16, 92.4, 67, 46.5, 1390, 13, 452),
    -- 여기서부터 흔들립니다 — 잠드는 시각이 밀리고 잠들기까지 길어집니다
    ( 7,  4880, 3370, 1620, 41, 351, 71, 200,  63, 17, 88.7, 71, 40.9, 1455, 28, 520),
    ( 6,  3960, 2730, 1450, 33, 322, 62, 186,  57, 17, 86.5, 73, 38.2, 1490, 35, 570),
    ( 5,  3210, 2210, 1310, 27, 298, 55, 174,  52, 17, 84.9, 75, 35.6, 1525, 44, 610),
    ( 4,  2470, 1700, 1180, 21, 271, 47, 160,  47, 17, 82.3, 78, 32.8, 1560, 52, 660),
    -- 고립 — 새벽에 잠들고 낮이 되어야 움직입니다
    ( 3,  1180,  810,  990, 11, 224, 34, 136,  38, 16, 77.5, 82, 28.4, 1620, 68, 740),
    ( 2,   740,  510,  920,  7, 196, 27, 121,  33, 15, 74.1, 85, 25.9, 1680, 79, 800),
    ( 1,   460,  320,  880,  4, 178, 22, 111,  29, 16, 71.6, 88, 23.7, 1710, 88, 850),
    ( 0,   390,  270,  860,  3, 171, 20, 107,  28, 16, 70.4, 89, 22.8, 1740, 95, 890)
) AS d(days_ago, steps, distance, calories, active_min,
       sleep_min, deep_min, light_min, rem_min, awake_min, efficiency, hr, hrv,
       sleep_at_min, onset_min, act_at_min)
