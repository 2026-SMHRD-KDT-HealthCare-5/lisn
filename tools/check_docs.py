...
빅데이터분석정의서는 **「검증했고 채택하지 않았다」는 근거를 담는
...
# 검사에서 뺄 문서.
#
#   빅데이터분석정의서  「검증했고 채택하지 않았다」는 **근거를 담는 문서**라
#                      모델명과 목표 수치가 남아 있어야 합니다. 그게 나머지
#                      문서에서 지운 이유의 출처입니다.
#   기획서_발표자료     **2026.08.04 에 개정 범위에서 빠졌습니다**(PM 결정).
#                      기획서 발표는 이미 끝났고, PL-20~PL-24 의 낡은 AI 설계
#                      서술(LSTM Autoencoder·LightGBM·85%/0.80)이 여기 남아
#                      있습니다. 범위 밖인 것을 계속 실패로 띄우면 **검사기가
#                      0 이 될 수 없어** 완료 판정 기준으로 못 씁니다.
#
# ⚠ **최종발표자료(8/28용)는 예외가 아닙니다**(2026.08.24 에 좁혔습니다).
#   전에는 예외가 `'발표자료'` 였는데 `'최종발표자료'` 가 그 조각을 포함해서
#   **8/28 발표자료까지 통째로 검사에서 빠져 있었습니다.** 새로 만든 별도
#   파일이라 PL-20~PL-24 서술이 없고 성과 지표도 실측 최신이라, 검사를 받아야
#   낡은 수치가 섞였을 때 잡힙니다.
#   기획서 발표자료를 8/28 에 같이 쓰기로 하면 그 6장을 먼저 고치고 이 예외도
#   마저 빼세요.
#
# ⚠ **접두사로 판정하지 않습니다.** 예전에는 `03_` 으로 걸렀는데,
#   2026.08.03 에 원본 파일명에서 번호 접두사가 빠지면서 추출본 이름도
#   바뀌었습니다. 접두사로만 보면 **예외가 조용히 풀려** 빅데이터분석정의서의 모델명까지
#   실패로 잡힙니다. 파일명 조각으로 봅니다.
#
# ⚠ 그 «조각으로 본다»가 이번 사고의 원인이기도 합니다. 예외를 넓게 잡으면
#   **의도하지 않은 파일까지 조용히 빠집니다.** 새 예외를 넣을 때는 어떤
#   파일이 걸리는지 실제로 세어 보세요.
EXEMPT = ('빅데이터분석정의서', '기획서_발표자료')
...
def check_모델명():
    """폐기한 모델 이름이 빅데이터분석정의서 밖에 남아 있는지."""
    ...
    report('모델명', bad,
           '빅데이터분석정의서는 근거 문서라 예외입니다. 나머지에서는 전부 빠져야 합니다.')


def check_성능목표():
    """달성하지 못한 목표 수치가 빅데이터분석정의서 밖에 남아 있는지."""
    pats = [
    ...
    for f in extracted_files():
        for i, line in enumerate(f.read_text(encoding='utf-8').splitlines(), 1):
            if any(p.search(line) for p in pats):
                bad.append(f'{f.name}:{i}  {line.strip()[:70]}')
    report('성능목표', bad,
           '빅데이터분석정의서가 「도달하지 못해 채택하지 않음」으로 기록한 수치입니다.')
