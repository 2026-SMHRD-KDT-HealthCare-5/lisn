...
def bench_chat(token, runs, utterance, label, budget):
    ...
    for _ in range(runs):
        ...
        xs.append(ms)
        actions.add((body.get('risk') or {}).get('action'))
    stat(label, xs, budget)
...
def main():
    ...
    bench_chat(token, args.runs, '오늘 회사에서 신경 쓰이는 일이 많았어요.',
               'NFR-DV-001 일반 발화', 3000)
    # high 키워드 '죽고 싶' 이 들어간 발화라야 1차 필터까지 함께 검증됩니다.
    bench_chat(token, args.runs, '요즘 정말 죽고 싶다는 생각이 들어요.',
               'NFR-TS-001 위기 발화', 3000)

    print('\n' + '=' * 78)
    print('측정하지 않은 것')
    print('  NFR-AI-001  위기 판정 재현율 90%·F1 0.85 — **고정 평가셋 200건이 없습니다**')
