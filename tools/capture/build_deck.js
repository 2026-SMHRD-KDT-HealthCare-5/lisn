...
async function main() {
  ...
  // ============================================================
  {
    ...
    const steps = [
      ...
      ['02', '평소와 얼마나 다른가', '사용자 본인의 평소(14일)와 비교해 이탈을 봅니다. 감정을 분류하는 것이 아니라, 그 사람 기준의 변화를 봅니다.'],
  ...
  // ============================================================
  {
    ...
    const weeks = [
      ['1주', '요구사항정의 · 산출물 5종 초안', '기업 브리프 대조 · DB 스키마 설계'],
  ...
  // ============================================================
  {
    ...
    const design = [
      ['설계 결정 1', '키워드 규칙을 비즈니스 서버 안에 둔다', 'AI 서버·외부 LLM이 죽어도 위기 탐지가 멈추면 안 됩니다(NFR-DV-003). 로컬 함수라 공급자와 무관하게 동작합니다.'],
      ['설계 결정 2', 'LLM 판정과 응답 생성을 병렬로 부른다', '순차 호출이면 왕복이 두 번 쌓여 3초 응답 요건을 못 지킵니다. asyncio.gather로 동시에 보냅니다.'],
      ['설계 결정 3', '스트리밍을 쓰지 않는다', 'CRITICAL로 확정되면 만들어 둔 응답을 버려야 하는데, 스트리밍으로 이미 흘려보낸 글자는 되돌릴 수 없습니다.'],
    ];
    let y = 1.35;
    const rh = 1.55;
  ...
  // ============================================================
  {
    ...
    const good = [
      ['검증 없이 채택하지 않았다', '학습 모델 채택에 26회, 위기 판정 개선에 4회차 재측정. "동작한다"가 아니라 "재봤더니 낫다"만 반영했습니다.'],
      ['문서-구현 갭을 스스로 찾아 닫았다', '요구사항 갭 9건 전부 해소. 마지막 갭(채팅 위기가 관제에 안 뜨던 것)은 발표 준비 중 직접 찾았습니다.'],
      ['brand.json·schema.sql — 정본을 하나로', '색·글꼴·스키마 값을 여러 곳에 반복 적지 않아, 나중에 값이 갈리는 사고 자체를 구조로 막았습니다.'],
    ];
    let gy = 2.0;
    good.forEach(([h, b]) => {
    ...
    const bad = [
      ['실기기 검증이 남아 있다', 'Health Connect·FCM 모두 에뮬레이터·자격증명 초기화까지만 확인했습니다. 실기기 하나면 반나절 안에 끝나는 검증인데, 확보하지 못했습니다.'],
      ['평가셋에 holdout이 없다', '재현율 0.946은 이 211건 안에서의 값입니다. 라벨링에 관여하지 않은 사람이 새로 쓴 문장으로 재검증하는 게 다음 단계입니다.'],
      ['AI 모델 표본이 이 분야 중앙값보다 작다', '이 분야 표본 중앙값이 60.5명인데 저희는 62명입니다. 42편 중 외부 검증은 1편뿐이라는 것도 함께 봐야 할 맥락입니다.'],
    ];
    let by = 2.0;
    bad.forEach(([h, b]) => {
