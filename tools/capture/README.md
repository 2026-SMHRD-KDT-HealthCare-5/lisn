...
### ⚠ 합성음은 두 번 시도했고, 두 번 다 사람 목소리로 돌아갔습니다 (2026.08.22)

**1차 — Windows 내장 한국어 음성(Heami, WinRT).** 기술적으로는 동작했지만
**결과가 기괴해서 기각**했습니다.
...
**2차 — ElevenLabs (Sarah, Free 플랜).** 품질은 훨씬 나았지만
**최종 결정은 「일단 영상만 두고 직접 녹음 하는게 낫겠다」였습니다.**
Sarah 가 원고를 목표 시간의 절반 이하로 읽어, 구간에 따라 화면만 남는
여백이 20초대까지 생겼기 때문입니다.

- `gen_elevenlabs.py` 로 재생성할 수 있습니다. 키/음성ID 는 로컬 파일에서
  읽습니다 — `secrets/elevenlabs_api_key.txt` 를 채우고 돌리세요.
- 처음 요청받은 음성(Seulki, 전문 보이스)은 **Free 플랜 API 에서
  막혔습니다** — "Free users cannot use library voices via the API".
  계정 기본 제공(premade) 목록 중 Sarah 로 대체했습니다.
- `build_video.sh` 는 **기본이 무음**입니다. 다시 얹으려면
  `NARRATE=1 bash build_video.sh`.

**결론 — 사람이 읽는 것이 가장 낫습니다.** 원고는 구간 길이에 맞춰
재 놓았습니다.
